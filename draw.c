...
/*   Updated: 2020/02/12 13:19:31 by rhoorntj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_menu(t_fdf *data)
{
	...
	mlx_string_put(data->mlx_ptr, data->win_ptr, 10, 80, 0xFFFFFF, "Projection");
	mlx_string_put(data->mlx_ptr, data->win_ptr, 150, 80, 0xFFFFFF, "ISO = 1 PARA = 2");
	mlx_string_put(data->mlx_ptr, data->win_ptr, 10, 100, 0xFFFFFF, "Color change");
...
int		start_map(t_fdf *data)
{
	if(!(data->mlx_ptr = mlx_init()))
		return(-1);
	if(!(data->win_ptr = mlx_new_window(data->mlx_ptr, W_WIDTH, W_HEIGHT, "FDF")))
		return(-1);
	data->p = 0;
	data->col = 1;
	data->pixel = 0;
